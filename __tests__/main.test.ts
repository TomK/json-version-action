...
beforeAll(() =>
{
  ...
  jest.spyOn(core, 'setOutput').mockImplementation((name, value) => outputs[name] = value);
...
describe('run', () =>
{
  ...
  it('checks package.json', async () =>
  {
    ...
    expect(errMock).toHaveBeenCalledTimes(1);
    expect(errMock).toHaveBeenCalledWith(Error('Could not find file package.json'));
  });
  ...
  it('checks composer.json', async () =>
  {
    ...
    expect(errMock).toHaveBeenCalledTimes(1);
    expect(errMock).toHaveBeenCalledWith(Error('Could not find file composer.json'));
  });
...
function mockContent(data: object)
{
  if ('content' in data)
  {
    data['content'] = Buffer.from(JSON.stringify(data['content'])).toString('base64');
  }
